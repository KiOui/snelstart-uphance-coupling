...
abstract class SUCAPIClient {
	...
	protected function make_exception( array|WP_Error $response, string $url ): SUCAPIException {
		...
		return new SUCAPIException(
			...
			$url . ":\n " . $msg,
			null,
			gettype( wp_remote_retrieve_headers( $response ) ) === "array" ?: wp_remote_retrieve_headers( $response )->getAll(),
		);
	}
	...
	protected function create_querystring( array $queries ): string {
		$queries = array_filter( $queries, function ( $query ) { return isset( $query ) && $query !== ""; } );
		if ( sizeof( $queries ) > 0 ) {
			$return_str = "";
			$prepend = "?";
